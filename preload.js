  ...
  $("#formDevice").on('submit', async (event) => {
    ...
    fs.readFile(inoPath, { encoding: 'utf-8' }, (err, data) => {
      ...
      fs.writeFile(inoPath, replaceData, err => {
        ...
        console.log('[INFO] Building ...')
        let terminalBuild = exec(`cd ${buildFolder} && arduino-cli compile -p ${port} --fqbn=esp8266:esp8266:nodemcuv2:xtal=80,vt=flash,exception=legacy,ssl=all,eesz=4M2M,led=2,ip=lm2f,dbg=Disabled,lvl=None____,wipe=none,baud=${baud} ${folderName}`, { windowsHide: true, maxBuffer: 1024 * 1024 * 500 }, (err) => {
          if (err) {
            console.log('[ERROR] ', err)
            fs.writeFile(inoPath, oriData, (err) => {
              if (err) console.log('[ERROR] ', err)
        ...
        terminalBuild.on('close', (code) => {
          ...
          console.log('[INFO] Start Uploading ...');
          let terminalUpload = exec(`cd ${buildFolder} && arduino-cli upload -p ${port} --fqbn=esp8266:esp8266:nodemcuv2:xtal=80,vt=flash,exception=legacy,ssl=all,eesz=4M2M,led=2,ip=lm2f,dbg=Disabled,lvl=None____,wipe=none,baud=${baud} ${folderName}`, { windowsHide: true, maxBuffer: 1024 * 1024 * 500 }, (err) => {
            if (err) console.log('[ERROR] ', err)
          })
